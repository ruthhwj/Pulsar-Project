...
int pgopenoutputfile(char *filename)
{
  if(ppgdevice_internal != NULL)
    fclose(ppgdevice_internal);
  ppgdevicename_internal[0] = 0;
  ppgdevice_internal = fopen(filename, "w");
  if(ppgdevice_internal == NULL) {
    fflush(stdout);
    fprintf(stderr, "ERROR pgoutputfile: Cannot open '%s'\n", filename);
    return 0;
  }
  strcpy(ppgdevicename_internal, filename);
  ppg_nextaux_internal = 1;
  return 1;
}
...
void ppggray(const float *a, int idim, int jdim, int i1, int i2, int j1, int j2, float fg, float bg, const float *tr)
{
  ...
  if(ppgdevice_internal == NULL)
    ...
    if(fout == NULL) {
      fflush(stdout);
      fprintf(stderr, "ERROR ppggray: Cannot open %s\n", filename);
      exit(0);
    }
    ret = fwrite(a, sizeof(float), idim*jdim, fout);
    if(ret != idim*jdim) {
      fflush(stdout);
      fprintf(stderr, "ERROR ppggray: Write error to %s (%ld != %ld)\n", filename, ret, (long)idim*jdim);
      exit(0);
    }
    fclose(fout);
    fprintf(ppgdevice_internal, "pggray %s %d %d %d %d %d %d %f %f %f %f %f %f %f %f\n", filename, idim, jdim, i1, i2, j1, j2, fg, bg, tr[0], tr[1], tr[2], tr[3], tr[4], tr[5]);
  }
}
...
void ppgerry(int n, const float *x, const float *y1, const float *y2, float t)
{
  if(ppgdevice_internal == NULL)
    ...
    fflush(stdout);
    fprintf(stderr, "ERROR ppgerry not defined in pgplotinterpretor mode\n");
    /* Could simply write out one at the time */
  }
}
...
void ppgpoly(int n, const float *xpts, const float *ypts)
{
  ...
  if(ppgdevice_internal == NULL)
    ...
    /*    fprintf(stderr, "ERROR ppgpoly not defined in pgplotinterpretor mode\n"); */
    sprintf(filename, "%s.aux%03d", ppgdevicename_internal, ppg_nextaux_internal++);
    fout = fopen(filename, "wb");
    if(fout == NULL) {
      fflush(stdout);
