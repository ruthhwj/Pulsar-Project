...
static int psrfits_weightmode = 0;   // 0=default, 1=Ignore weights, 2=Ignore weights, except when 0, 3=Multiply data with weights
...
int writeFITSpulse(datafile_definition datafile, long pulsenr, int polarization, int freq, int binnr, long nrSamples, float *pulse, verbose_definition verbose)
{
  ...
  /* Check if we're writing a receiver model, in which case there is no subint table. */
  if(datafile.gentype == GENTYPE_RECEIVERMODEL || datafile.gentype == GENTYPE_RECEIVERMODEL2) {
    ...
    for(n = binnr; n < binnr+nrSamples; n++) { 
      /*      i = 1+polarization*datafile.NrFreqChan+freq;*/
	...
	i = 1+freq*(datafile.NrPols-2)+polarization;
      ...
      /*      printf("XXXXXXXXXXXXXXX (pulse=%ld freq=%d pol=%d i=%ld).\n", pulsenr, freq, polarization, i); */
      if(n == 0) { /* Write data value */
    ...
    // Check if writing out part of a pulse, which is not working, because the scale/offset needs to be determined using the full period.
    double period;
    int ret;
	...
	long , i2
	int bitoff, anynul;
	unsigned char bytevalue, bytevalue2;
	for(i = 0; i < nrSamples; i++) {
	i2 = polarization*datafile.NrBins*datafile.NrFreqChan+(binnr+i)*datafile.NrFreqChan+freq;
	i2 *= 4;
	bitoff = i2 % 8;
	i2 /= 8;
	if(fits_read_col(datafile.fits_fptr, TBYTE, 18, 1+pulsenr, i2+1, 1, NULL, &bytevalue, &anynul, &status)) {
    fflush(stdout);
	printerror(verbose.debug, "ERROR writeFITSpulse: Error readubg data.");
	fits_report_error(stderr, status); 
	}
	bytevalue2 = data[i]*16/30000;
	if(bitoff)
	bytevalue2 *= 16;
	bytevalue += bytevalue2;
	if(fits_write_col(datafile.fits_fptr, TBYTE, 18, 1+pulsenr, i2+1, 1, &bytevalue, &status) != 0) {
    fflush(stdout);
	printerror(verbose.debug, "ERROR writeFITSpulse: Error writing data.");
	fits_report_error(stderr, status);
    ...
    /* Assume that weight is already applied to data */
    weight = 1;
    ...
    /*      if(!fits_read_col(datafile->fits_fptr, TFLOAT, 16, 1+pulsenr, 1, datafile->NrFreqChan, NULL, &datafile->offsets[n*datafile->NrPols*datafile->NrFreqChan+p*datafile->NrFreqChan], &anynul, &status)) {*/
    
    /*
      if(fits_movabs_hdu(datafile.fits_fptr, 9, &ncols, &status) != 0) {
    fflush(stdout);
      printerror(verbose.debug, "ERROR writeFITSpulse: Cannot move to subint HDU.");
      return 0;
      }
      fits_get_num_rows(datafile.fits_fptr, &nrows, &status);
      fits_get_num_cols(datafile.fits_fptr, &ncols, &status);
      ...
      if(ncols < 19) {
    fflush(stdout);
      printerror(verbose.debug, "ERROR writeFITSpulse: Table has not the correct size, was writePSRFITSHeader not called?");
      return 0;
      }*/
  }
...
int readFITSpulse_receivermodel(datafile_definition *datafile, long pulsenr, int polarization, int freq, int binnr, long nrSamples, float *pulse, verbose_definition verbose)
{
  ...
  /* Check for request of reading in chisq column */
  if(datafile->gentype == GENTYPE_RECEIVERMODEL2 && polarization == datafile->NrPols-2) {
	...
	pulse[0] = datapoint;
	pulse[1] = 0;             /* There are no errors on chisq values */
      }else {
	fflush(stdout);
	...
	return 0;
      ...
      pulse[0] = 0;             /* There are no errors on chisq values */
    }else {
      fflush(stdout);
      printerror(verbose.debug, "ERROR readFITSpulse_receivermodel: Nr of bins is expected to be 1 or 2");
      return 0;
  ...
  /* Check for request of reading in nfree column */
  ...
  /* Calculate the actual number of receiver parameters in the file */
  ncpar = datafile->NrPols;
  if(datafile->gentype == GENTYPE_RECEIVERMODEL2)
    ncpar -= 2;

  /* If we reach this stage, we want to read in the receiver solution itself */

  /* Read error value if required */
...
int readFITSpulse(datafile_definition *datafile, long pulsenr, int polarization, int freq, int binnr, long nrSamples, float *pulse, verbose_definition verbose)
{
  ...
  long i, istart, bstart, bsample;   /* , iend, bend, isample */
  int *data;
  ...
  int weightmode;
  weightmode = 0;  // 0=default, 1=Ignore weights, 2=Ignore weights, except when 0, 3=Multiply data with weights
  ...
  */
  ...
  if(datafile->NrFreqChan == 1) {
    // If only one frequency channel, frequency averaging must have happened. In that case, the default would be to IGNORE the weights. In psrchive, the intensity as written out is the AVERAGE. Having different number of channels added in different subints would lead to for example jumps in the baseline. Also, if you're interested in subpulse modulation, multiplying with the weight would result in arbitrary jumps in intensity. However, if you're interested in forming a high S/N pulse profile the weights should really be included to form the weighted average.
    if(psrfits_weightmode == 0) {
      if(datafile->weight_stats_differentweights && pulsenr == 0 && polarization == 0 && freq == 0 && binnr == 0) {  // Only produce warning if there are actually different weights, otherwise the choice is not really relevant
  ...
  /*  if(freq != 0) {
    fflush(stdout);
    printerror(verbose.debug, "ERROR readFITSpulse: Cannot handle frequency resolution in PSRFITS");
    return 0;
    }*/
  data = (int *)malloc(datafile->NrBins*sizeof(int));
  if(data == NULL) {
  ...
  if(psrfits_use_weighted_freq) {     // Don't think this part of the code is used anymore
    ...
    //      fflush(stdout);
    //      printerror(verbose.debug, "ERROR readFITSpulse: Memory allocation error");
    //      return 0;
    //    }
    double freq;
    ...
    if(set_bandwidth(datafile, 0.0, verbose) == 0) { // Must be defined elsewhere?
      fflush(stdout);
  ...
  /*  if(!fits_read_col(datafile->fits_fptr, TINT, 18, 1+pulsenr, 1+polarization*(datafile->NrBins)+binnr, nrSamples, NULL, data, &anynul, &status)) { */

  ret = 0;
  if(datafile->NrBits == 16) {
    /* First sample to be read*/
    istart = polarization*datafile->NrBins*datafile->NrFreqChan+freq*datafile->NrBins+binnr;
    /* Last sample to be read */
    ...
    /* Read in data byte for byte, this is because data is written
       with freq in the inner loop, which means that you should really
       use a function readFITSfreq() instead. */
    ret = 1;
  }else {
    fflush(stdout);
  ...
  if(ret == 1) {
    ...
    for(i = 0; i < nrSamples; i++) {
      //      if(pulsenr == 0) {
      //	fprintf(stderr, "XXXX bin=%ld data=%d scale=%f offset=%f\n", i, data[i], datafile->scales[pulsenr*datafile->NrPols*datafile->NrFreqChan+polarization*datafile->NrFreqChan+freq], datafile->offsets[pulsenr*datafile->NrPols*datafile->NrFreqChan+polarization*datafile->NrFreqChan+freq]);
      //      }

      /*           pulse[i] = datafile->scales[pulsenr*datafile->NrPols+polarization]*(data[i]) + datafile->offsets[pulsenr*datafile->NrPols+polarization];  */
      if(datafile->NrBits == 16) {
	...
	//	if(pulsenr == 0) {
	//	  fprintf(stderr, "XXXX scaled data=%f\n", pulse[i]);
	//	  exit(0);
	//	}
      }else if(datafile->NrBits == 2 || datafile->NrBits == 4 || datafile->NrBits == 8) {

	/* First sample to be read*/
	// Order is sample (outer loop), polarization, freq channel (inner loop)
	// Assumed order used to be: polarization, sample, freq
	//	istart = polarization*datafile->NrBins*datafile->NrFreqChan+(binnr+i)*datafile->NrFreqChan+freq;
	istart = (binnr+i)*datafile->NrPols*datafile->NrFreqChan+polarization*datafile->NrFreqChan+freq;
	/* bstart is start bit nr */
	bstart = istart*datafile->NrBits;
	/* istart is first byte to be read */
	istart = bstart/8;

	/*    fprintf(stderr, "\nXXXXXX %ld %ld %ld\n\n", 1+pulsenr, 1+istart, iend-istart+1); */
	...
	if(datafile->NrBits == 4) {
	  if(bsample == 0) {
	    ...
	    /*	  fprintf(stderr, "XXXXXX %d\n", singlebyte); */
	  }else {
	    fflush(stdout);
	    printerror(verbose.debug, "ERROR readFITSpulse: bin shift error.");
	    return 0;
	  }
	}else if(datafile->NrBits == 8) {   /* Nothing needs to be done */
	...
	/*	printf("XXXXX %d: %f %f\n", freq, datafile->scales[pulsenr*datafile->NrPols*datafile->NrFreqChan+polarization*datafile->NrFreqChan+freq], datafile->offsets[pulsenr*datafile->NrPols*datafile->NrFreqChan+polarization*datafile->NrFreqChan+freq]);*/
	
	/*	pulse[i] = singlebyte; */
      }
      if(weightmode == 3 && psrfits_absweights == 0) {
	/*	if(freq == 749 && polarization == 1) {
	  printf("XXXXXX %ld: %e %e %e\n", pulsenr*datafile->NrFreqChan+freq, pulse[i], datafile->scales[pulsenr*datafile->NrPols*datafile->NrFreqChan+polarization*datafile->NrFreqChan+freq], datafile->weights[pulsenr*datafile->NrFreqChan+freq]);
	  }*/
	pulse[i] *= datafile->weights[pulsenr*datafile->NrFreqChan+freq];
	/*	if(datafile->weights[pulsenr*datafile->NrFreqChan+freq] == 0)
	  pulse[i] = 0;*/
...
int readPSRCHIVE_ASCIIHeader(datafile_definition *datafile, verbose_definition verbose)
{
  ...
  // Folding is implied, otherwise cannot change period on command line
  datafile->isFolded = 1;
  // Fixed period/sampling is implied
...
// Verbose determines the nr of spaces before verbose information
// Debug increases what is printed out
...
int readPSRFITSscales(datafile_definition *datafile, verbose_definition verbose)
{
  ...
  //  printf("Allocating weights table (%ld floats)\n", datafile->NrSubints*datafile->NrFreqChan);
  ...
  for(n = 0; n < datafile->NrSubints; n++) {
    for(p = 0; p < datafile->NrPols; p++) {
      /*      if(!fits_read_col(datafile->fits_fptr, TFLOAT, 17, 1+n, 1, datafile->NrPols, NULL, &datafile->scales[n*datafile->NrPols], &anynul, &status)) {*/
      ...
      /*      if(!fits_read_col(datafile->fits_fptr, TFLOAT, 16, 1+n, 1, datafile->NrPols, NULL, &datafile->offsets[n*datafile->NrPols], &anynul, &status)) { */
      ...
      if(p == 0) {
	// 1+fullscales*p*datafile->NrFreqChan, datafile->NrFreqChan = 1, so read weights of all freq. channels in one go.
    ...
    /*
    int i;
    for(i = 0; i < datafile->NrPols; i++) {
      if(i == 0)
	printf("scale= ");
      printf("%e ", datafile->scales[n*datafile->NrPols+i]);
    }
    printf("\n"); 
    for(i = 0; i < datafile->NrPols; i++) {
      if(i == 0)
	printf("offset= ");
      printf("%e ", datafile->offsets[n*datafile->NrPols+i]);
    }
    printf("\n"); 
    for(i = 0; i < datafile->NrPols; i++) {
      if(i == 0)
	printf("weight= ");
      printf("%e ", datafile->weights[n*datafile->NrPols+i]);
    }
    printf("\n");
    */
    ...
    for(i = 0; i < datafile->NrPols*datafile->NrFreqChan; i++) {
      printf("XXXXXX %ld - scale= ", n*datafile->NrPols+i);
      printf("%e ", datafile->scales[n*datafile->NrPols+i]);
      printf("offset= ");
      printf("%e ", datafile->offsets[n*datafile->NrPols+i]);
      printf("weight= ");
      printf("%e \n", datafile->weights[n*datafile->NrPols+i]);
    }
    */
    ...
    /*    for(p = 0; p < datafile->NrPols; p++) {
      printf("YYYY %e\n", datafile->scales[n*datafile->NrPols*datafile->NrFreqChan+p*datafile->NrFreqChan+749]);
      }*/
  }
...
int writePSRFITSHeader(datafile_definition *datafile, verbose_definition verbose)
{
  ...
  */
  ...
  //    fflush(stdout);
  //    printerror(verbose.debug, "ERROR writePSRFITSHeader: Frequency channels need to be equally separated.");
  //    return 0;
  //  }
  ...
  */
  ...
  //  printf("XXXXX gentype written is: %s\n", returnGenType_str(datafile->gentype));
  if(fits_write_key(datafile->fits_fptr, TINT, "PS_GTYPE", &datafile->gentype, "Describes the type of data in file", &status) != 0) {
    fflush(stdout);
    printerror(verbose.debug, "ERROR writePSRFITSHeader: Cannot write keyword.");
    return 0;
  }
  //  if(fits_write_key(datafile->fits_fptr, TDOUBLE, "PS_TSUB", &datafile->fixedtsub, "Length of each subintegration in seconds", &status) != 0) {
  //    fflush(stdout);
  //    printerror(verbose.debug, "ERROR writePSRFITSHeader: Cannot write keyword.");
  //    return 0;
  //  }
  ...
  // tform for NBIN was 1I, but is not always long enough (search mode), so changed it to 1J
  ...
  /*
  char *ttypes_history[] = {"DATE_PRO", "PROC_CMD", "SCALE", "POL_TYPE", "NSUB", "NPOL", "NBIN", "NBIN_PRD", "TBIN", "CTR_FREQ", "NCHAN", "CHAN_BW", "DM", "RM", "PR_CORR", "FD_CORR", "BE_CORR", "RM_CORR", "DEDISP", "DDS_MTHD", "SC_MTHD", "CAL_MTHD", "CAL_FILE", "RFI_MTHD", "IFR_MTHD"};
  char *tform_history[] = {"24A", "80A", "8A", "8A", "1I", "1I", "1I", "1I", "1D", "1D", "1I", "1D", "1D", "1D", "1I", "1I", "1I", "1I", "1I", "32A", "32A", "32A", "32A", "32A", "32A"};
  char *tunit_history[] = {"", "", "", "", "", "", "", "", "s", "MHz", "", "MHz", "CM-3", "RAD", "", "", "", "", "", "", "", "", "", "", ""};
  */

  char *ttypes_history2[] = {"DATE_PRO", "USER", "HOSTNAME", "PROC_CMD"};
  char *tform_history2[] = {"24A", "24A", "32A", "1024A"};
  char *tunit_history2[] = {"", "", "", ""};
  /* Create an alternative history table with only two columns. If the
     PSRFITS HISTORY table is not filled in correctly, PSRCHIVE will
     not recognize for instance the frequency etc. Better not to use
     the PSRFITS history for now. I made the command line much larger
     (1024 characters) and if the command line is longer it will spawn
  ...
  */
  ...
  if(datafile->gentype != GENTYPE_RECEIVERMODEL && datafile->gentype != GENTYPE_RECEIVERMODEL2) {
    ...
    if(datafile->gentype != GENTYPE_SEARCHMODE && datafile->isFolded == 1 && ret == 0 && period >= 0) {
      ...
      //      fprintf(stderr, "XXXX nr samples per subint=%s\n", dummy_txt3);
    }else {   /* search mode */
	...
	dummy_double = -1;
      ...
      if(ret_prd == 1) {  // To make output the same as in previous version, but it is strange that period is written out in the first place in search mode.
	dummy_double = -1;
      }
	...
	//	dummy_float = datafile->freq_cent - 0.5*(datafile->NrFreqChan-1.0)*get_channelbw(*datafile) + j*get_channelbw(*datafile);
	dummy_double = get_weighted_channel_freq(*datafile, i, j, verbose);
	//	printf("XXXXXX %ld %f %lf MHz\n", j, dummy_float, dummy_double);
  ...
  }else if(datafile->gentype == GENTYPE_RECEIVERMODEL || datafile->gentype == GENTYPE_RECEIVERMODEL2) { /* End of SUBINT table write routine */
    sprintf(dummy_txt, "%ldE", datafile->NrFreqChan);
    sprintf(dummy_txt2, "%ldE", datafile->NrFreqChan*datafile->NrPols);
    ...
    dummy_int = datafile->NrPols;
    /* Last two "polarization" channels do not contain coupling parameters */
    if(datafile->gentype == GENTYPE_RECEIVERMODEL2)
      dummy_int -= 2;
    ...
    /* Fill frequency channel table and weights table and errors */
    for(i = 0; i < datafile->NrSubints; i++) {
      for(j = 0; j < datafile->NrFreqChan; j++) {
	//	dummy_float = datafile->freq_cent - 0.5*(datafile->NrFreqChan-1.0)*get_channelbw(*datafile) + j*get_channelbw(*datafile);
	...
	dummy_float = 1e-5;
	/* These errors will be overwritten, if they are provided later. */
  ...
  dummy_int = datafile->NrSubints*datafile->NrPols*datafile->NrFreqChan;
...
// Returns the POLTYPE, nothing changed in header
...
/*
  Updates centre frequency/channel bw using the DAT_FREQ in the fits table currently selected.
 */
...
/*
  If readnoscales is set, the scales/offsets/weights of the subints
  are not read in. Useful when only interested in header information.

  If debug is set, more information is printed.

These parameters are undefined
  float period  (actually, now doing system call to use vap to get it)
  char institute[100], scanid[100];
*/
int readPSRFITSHeader(datafile_definition *datafile, int readnoscales, int nowarnings, verbose_definition verbose)
{
  char card[FLEN_CARD], comment[FLEN_COMMENT], value[FLEN_VALUE], value_tmp[FLEN_VALUE];   /* Standard string lengths defined in fitsio.h */
  int status = 0;   /* CFITSIO status value MUST be initialized to zero! */
  ...
  gentypenotset = 1;
  tsubnotset = 1;
  issearch = 0;          // Set if OBS_MODE is set to be search mode
  tsubsettotobs = 0;
  version_major = 0;
  version_minor = 0;
  datafile->NrPols = -1;      // Default: unknown
  datafile->NrBins = -1;      // Default: unknown
  datafile->NrFreqChan = -1;  // Default: unknown
  bandwidth = 0.0;            // Default: unknown

  /*  fits_get_hdu_num(datafile->fits_fptr, &hdupos); */ /* Get the current HDU position */
  ...
  fits_get_hdrspace(datafile->fits_fptr, &nkeys, NULL, &status); /* get # of keywords */
  ...
  //  if(strcmp(value, "'        '") != 0) {
  //    sscanf(value, "'%s'", datafile->observatory);
  //    if(strlen(datafile->observatory) > 0)
  //      if(datafile->observatory[strlen(datafile->observatory)-1] == '\'')
  //	datafile->observatory[strlen(datafile->observatory)-1] = 0;
  //  }else
  ...
  //  if(strlen(value) > 0) {
  //    sscanf(value, "'%s'", datafile->psrname);
  //  }else {
  //    datafile->psrname[0] = 0;
  //  }
  //  for(i = 1; i < strlen(datafile->psrname); i++)
  ...
  //  if(strcmp(value, "'        '") != 0)
  //    sscanf(value, "'%s'", datafile->instrument);
  //  else
  ...
  if(fits_read_card(datafile->fits_fptr,"OBSFREQ", card, &status)) {
    fflush(stdout);
    if(nowarnings == 0) {
      // NOT CLEAR FROM DOCUMENTATION WHEN THIS PARAMETER WAS INTRODUCED. WASN'T DEFINED IN A 2.4 FILE FROM PARKES. FROM PSRCHIVE IT LOOKS LIKE IT WAS INTRODUCED IN VERSION 2.8
      if((version_major > 2) || (version_major == 2 && version_minor >= 8)) {
    ...
    if(ret != 1) {  // Probably because OBSFREQ = '*' = undefined
      dummy_double = 0;
    }
  }
  datafile->freqMode = FREQMODE_UNIFORM;
  ...
  if(dummy_double < 1 && (dummy_double < -1.1 || dummy_double > -0.9)) {
    ...
    datafile->freq_ref = dummy_double;  // Set the reference frequency to OBSFREQ. The actual freqs should come from the frequencies in the subint table, but this is the first guess for the actual frequencies in case those do not exist.
  }
  if(verbose.debug)
  ...
  /*
  if (fits_read_card(datafile->fits_fptr,"FD_SANG", card, &status)) {
    if(nowarnings == 0) {
    fflush(stdout);
    printwarning(verbose.debug, "WARNING readPSRFITSHeader (%s): FD_SANG keyword does not exist", datafile->filename);
}
    status = 0;
  }else {
    fits_parse_value(card, value, comment, &status);
    sscanf(value, "%f", &datafile->fd_sang);
  }
  if (fits_read_card(datafile->fits_fptr,"FD_XYPH", card, &status)) {
    if(nowarnings == 0) {
    fflush(stdout);
    printwarning(verbose.debug, "WARNING readPSRFITSHeader (%s): FD_XYPH keyword does not exist", datafile->filename);
}
    status = 0;
  }else {
    fits_parse_value(card, value, comment, &status);
    sscanf(value, "%f", &datafile->fd_xyph);
  }
  */
  ...
  if(strstr(value, "SEARCH") != 0) {
    ...
    gentypenotset = 0;
    /* Don't set dd_mode, doesn't work properly with PuMa format */
    /*    datafile->dd_mode = 1; */
  ...
  if(datafile->tsub_list != NULL) {
    //    printf("XXXXX FITS READ HEADER free %p\n", datafile->tsub_list);
    free(datafile->tsub_list);
  }
  datafile->tsub_list = (double *)malloc(sizeof(double));
  //  printf("XXXXX FITS READ HEADER alloc %p\n", datafile->tsub_list);
  if(datafile->tsub_list == NULL) {
    fflush(stdout);
  ...
  datafile->tsub_list[0] = 0;
//START REGION DEVELOP
  // Now no longer in use, but for backward compatibility still supported
  if(fits_read_card(datafile->fits_fptr,"PS_TSUB", card, &status)) {
    status = 0;
    if(fits_read_card(datafile->fits_fptr,"PS_OBS", card, &status)) {   // Previously total observation length was stored, so only for backward compatibility
      datafile->tsubMode = TSUBMODE_UNKNOWN;
      datafile->tsub_list[0] = 0;
      status = 0;
    }else {
      fits_parse_value(card, value, comment, &status);
      sscanf(value, "%lf", &(datafile->tsub_list[0]));
      tsubsettotobs = 1;
      datafile->tsubMode = TSUBMODE_FIXEDTSUB;
      tsubnotset = 0;
    }
...
//START REGION RELEASE
  ...
  if(fits_read_card(datafile->fits_fptr,"PS_DBASE", card, &status)) {
    ...
    //    if(verbose.debug) {
    //      fflush(stdout);
    //      printf("  readPSRFITSHeader (%s): Baseline subtraction state not defined in header\n", datafile->filename);
    //    }
  }else {
    fits_parse_value(card, value, comment, &status);
    sscanf(value, "%d", &dummy_int);
  ...
  if(fits_movnam_hdu(datafile->fits_fptr, BINARY_TBL, "HISTORY", 0, &status)) {
    ...
    datafile->isDePar = -1;
    if(periodnotset) {  // Only give warning if not already set to be search mode
      if(verbose.debug) {
	fflush(stdout);
    ...
    status = 0;
  }else {     // read parameters from HISROTY table
    ...
    if(datafile->NrFreqChan != -1) {  // We need to know the number of frequency channels to make use of channel bandwidth
	...
	status = 0;
      ...
      if((freq_history > -1.01 && freq_history < -0.99)) {  // The -1 is now written out as 1e10
	if(verbose.debug)
	...
	freq_history = 1e10;
      ...
      //	printf("  readPSRFITSHeader (%s): Frequency defined in history table = %lf (-1 or 1e10 is interpreted as infinity)\n", datafile->filename, freq_history);
      if(get_centre_frequency(*datafile, verbose) < 1) {   /* Centre frequency not correcly set, use the one from history table instead */
	//	datafile->freqMode = FREQMODE_UNIFORM;
	...
	if(verbose.debug)
      ...
      if(datafile->freq_ref < 1) {   /* Reference frequency not correcly set, use the one from history table instead */
  ...
  } // End of if history table exists

  if(get_centre_frequency(*datafile, verbose) < 1) {   /* Centre frequency not correcly set, try to get it from T2predictor/polyco */
    if(verbose.debug)
  ...
  if(get_centre_frequency(*datafile, verbose) < 1) {   /* Centre frequency not correcly set, try to get it from T2predictor/polyco */
	...
	printf("  readPSRFITSHeader (%s): T2PREDICT table does not exist\n", datafile->filename);
      //	fflush(stdout);
      //	printerror(verbose.debug, "readPSRFITSHeader (%s): Both T2PREDICT & POLYCO table do not exist! This table is used only to get a frequency. Should get it from DAT_FREQ column in SUBINT instead, but this is not implemented.\n", datafile->filename);
      //	return 0;
      status = 0;
    }else {
      fits_get_hdrspace(datafile->fits_fptr, &nkeys, NULL, &status); /* get # of keywords */
	...
	//	printwarning(verbose.debug, "WARNING readPSRFITSHeader (%s): found T2predictor", datafile->filename);
	sprintf(command, "XXX");
	char_ptrptr[0] = command;
	for(i = 1; i <= numrows; i++) {
	  fits_read_col (datafile->fits_fptr, TSTRING, colnum, i, 1, 1, 0, char_ptrptr, &anynul, &status);
	  /*	    printwarning(verbose.debug, "WARNING readPSRFITSHeader: %d %d/%ld '%s' %d", colnum, i, numrows, command, status); */
	  sscanf(command, "%s", dummy_txt);
	  if(strcmp(dummy_txt, "FREQ_RANGE") == 0) {
	    /*	      printwarning(verbose.debug, "WARNING readPSRFITSHeader: %s", command); */
  ...
  if(fits_movnam_hdu(datafile->fits_fptr, BINARY_TBL, "SUBINT", 0, &status)) {
	...
	return 0;
      ...
      if(nowarnings == 0) {
	// NOT CLEAR FROM DOCUMENTATION WHEN THIS PARAMETER WAS INTRODUCED. WASN'T DEFINED IN A 2.4 FILE FROM PARKES. FROM PSRCHIVE IT LOOKS LIKE IT WAS INTRODUCED IN VERSION 2.7
    ...
    if(fits_read_card(datafile->fits_fptr, "POL_TYPE", card, &status)) {
      // NOT CLEAR FROM DOCUMENTATION WHEN THIS PARAMETER WAS INTRODUCED. WASN'T DEFINED IN A 2.4 FILE FROM PARKES. FROM PSRCHIVE IT LOOKS LIKE IT WAS INTRODUCED IN VERSION 3.0
      if(nowarnings == 0) {
	fflush(stdout);
	...
	datafile->NrBins = nrbins;
      ...
      /* If only one bin, it should be search mode data. */
    if(datafile->NrBins == 1) {
	...
	//	printf("XXXX GOT NCH_FILE\n");
	fits_parse_value(card, value, comment, &status);
	ret = sscanf(value, "%ld", &(dummy_long));
      }
    }else {
      //      printf("XXXX GOT NCHAN\n");
      fits_parse_value(card, value, comment, &status);
      ret = sscanf(value, "%ld", &(dummy_long));
    }
    //    printf("XXXX %d: %s %ld\n", ret, value, dummy_long);
    if(ret != -1) {
      if(ret != 1) {
    ...
    if(tsubsettotobs) {  // Previously recorded tsub really is a tobs, but the nr of subints were not known yet at the time
    ...
    //    fflush(stdout);
    //    printwarning(verbose.debug, "WARNING readPSRFITSHeader (%s): The centre frequency is not necessarily the same as the reference frequency.", datafile->filename);

    if (status == END_OF_FILE)  status = 0; /* Reset after normal error */

    // Determine total duration of observation in order to determine gentype
    if(issearch) {
      datafile->tsubMode = TSUBMODE_FIXEDTSUB;
	...
	int ok;
	ok = 1;
	// If tsub is not set via a PS_... parameter (is now obsolete, so should no longer happen for new data), set tsub for each subint independently
	if(tsubnotset) {
	  datafile->tsubMode = TSUBMODE_TSUBLIST;
	  if(datafile->NrSubints == 1) {
	    datafile->tsubMode = TSUBMODE_FIXEDTSUB;  // If only one subint, it really is the same as a fixed tsub
	  }
	  if(datafile->tsub_list != NULL) {
	    //	    printf("XXXXX FITS READ HEADER free %p\n", datafile->tsub_list);
	    free(datafile->tsub_list);
	...
	for(i = 0; i < datafile->NrSubints; i++) {
	  if(fits_read_col(datafile->fits_fptr, TDOUBLE, colnum, 1+i, 1, 1, NULL, &(subint_duration), &anynul, &status)) {
	    ...
	    //	  printf("XXXXX: duration of subint: %lf (tot=%lf)\n", subint_duration, tot_duration);
	  }
	}
	if(ok) {
	  ...
	  testvalue = (tot_duration-expected_duration)/expected_duration;  // difference = 0 for sp data, > 0 if folded
	  //	printf("XXXXX: testvalue = %lf (tot=%lf)\n", testvalue, tot_duration);
	  if(testvalue < 0.01) {
	    if(verbose.debug) {
	      fflush(stdout);
	    ...
	    //	    if(tsubnotset) {
	    //	      datafile->tsubMode = TSUBMODE_FIXEDTSUB;
	    //	      datafile->fixedtsub = tot_duration/(double)datafile->NrSubints;
	    tsubnotset = 0;
	    //	    }
	    ...
	    // Period not set, so there is a real problem
	    if(period != 0) {
	      if(nowarnings == 0) {
		...
		printwarning(verbose.debug, "WARNING readPSRFITSHeader (%s): Period does not appear to be set. Observation duration cannot be compared to number of subints.", datafile->filename);
	      ...
	      // Do keep subints as having zero length when reading in ascii file with undetermined period.
	      //	      datafile->tsubMode = TSUBMODE_UNKNOWN;
	      if(gentypenotset) {
		fflush(stdout);
		printf("readPSRFITSHeader (%s): Without an observation duration the gentype is not determined.\n", datafile->filename);
    ...
    /*   Dump header, can be usefull for debugging purposes */
    /*
      if(hdupos == 7) {
      for (ii = 1; ii <= nkeys; ii++) { 
      if (fits_read_record(datafile->fits_fptr, ii, card, &status))  
      break;
      printf("%s\n", card);
      }
      }
    */
    ...
    /*    fits_movrel_hdu(datafile->fits_fptr, 1, NULL, &status); */ /* try to move to next HDU */
    
    //    if (status == END_OF_FILE)  status = 0; /* Reset after normal error */

    if((dmnotset || rmnotset) && issearch == 0) {
      /* Look for the ephemeris */
    ...
    /* Move back to SUBINT table, as it is assumed that we are in data table when reading in the data */
    if(fits_movnam_hdu(datafile->fits_fptr, BINARY_TBL, "SUBINT", 0, &status)) {
      fflush(stdout);
    ...
    if (status) {
      fflush(stdout);
      fits_report_error(stderr, status); /* print any remaining error message */
    }
  ...
  if(nodata) {  /* No subint data found, so try if it is a receiver solution */
    if(fits_movnam_hdu(datafile->fits_fptr, BINARY_TBL, "FEEDPAR", 0, &status)) {
      if(verbose.verbose) 
    ...
    fits_parse_value(card, value, comment, &status);
    sscanf(value, "%ld", &(datafile->NrPols));           /* Set nr polarizations to be the number of fitted parameters */
    ...
    datafile->gentype = GENTYPE_RECEIVERMODEL2;  /* Default: chisq and nfree columns are present */
    ...
    datafile->NrBins = 2;   /* Default: Parameters + errors are in file */
    if(fits_get_colnum (datafile->fits_fptr, CASEINSEN, "DATAERR", &colnum, &status)) { 
      datafile->NrBins = 1;
      status = 0;
    ...
    if (fits_read_card(datafile->fits_fptr,"OBSBW", card, &status)) {
      fflush(stdout);
      printerror(verbose.debug, "ERROR readPSRFITSHeader (%s): OBSBW keyword does not exist", datafile->filename);
      return 0;
    }
    fits_parse_value(card, value, comment, &status);
    sscanf(value, "%lf", &(datafile->bw));
    // Move back to FEEDPAR table, as it is assumed that we are in data table when reading in the data
    if(fits_movnam_hdu(datafile->fits_fptr, BINARY_TBL, "FEEDPAR", 0, &status)) {
      fflush(stdout);
      printerror(verbose.debug, "ERROR readPSRFITSHeader (%s): FEEDPAR table does not exist!", datafile->filename);
      return 0;
    } 
    */
...
int constructFITSsearchsubint(datafile_definition datafile, float *data, int subintnr, unsigned char **subintdata, float **scales, float **offsets, int alreadyscaled, int allocmem, int destroymem, verbose_definition verbose)
{
  ...
  if(datafile.NrBits == 4) {
    ...
    bitofffac[4] = 4;
    bitofffac[6] = 1;
  }else if(datafile.NrBits != 8) {  /* Replace this by 16 is 16 bit search mode gets implemented */
    fflush(stdout);
  ...
  for(f = 0; f < datafile.NrFreqChan; f++) {
	...
	/* patrick: I switched around polarization and bin number, this appears to work when there is polarization. */
...
int readPSRCHIVE_ASCIIfilepulse(datafile_definition datafile, long pulsenr, int polarization, int freq, int binnr, long nrSamples, float *pulse, verbose_definition verbose)
{
  ...
  for(n = 0; n < datafile.NrSubints; n++) {
    /*    if(verbose.verbose  && verbose.nocounters == 0) printf("pulse %ld/%ld\r", n+1, datafile.NrSubints); */
    for(f = 0; f < datafile.NrFreqChan; f++) {
      for(i = 0; i < datafile.NrBins; i++) {
...
int writeHistoryFITS(datafile_definition datafile, verbose_definition verbose)
{
  ...
  curhistoryEntry = &(datafile.history);  // There should be at least one entry defined. This is an older history line, or the pressent command (could in principle be empty after cleanPSRData).
  do { // Loop over existing history entries in datafile struct
    if(curhistoryEntry->cmd != NULL)
      cmd_ptr = curhistoryEntry->cmd;
    else
      cmd_ptr = questionmark;
    nstart = 0;    // Current character in command line to write out
    long totallength = strlen(cmd_ptr);
    do {  // Loop over nr of fits lines required to write out long command lines
      fits_get_num_rows(datafile.fits_fptr, &nrows, &status);
	...
	return 0;
      ...
      if(nstart == 0) { // First line in fits file corresponding to current history entry
	...
	if(fits_write_col(datafile.fits_fptr, TSTRING, colnum_hostname, nrows, 1, 1, &str_ptr, &status) != 0) {
      ...
      } // End of writing out date/user/hostname

      //      fprintf(stderr, "XXXXX='%s'\n", cmd_ptr);
      txt2[1024] = 0; // Make sure the string is null terminated
      strncpy(txt2, cmd_ptr, 1024);
      n = strlen(txt2);
      cmd_ptr += n; // Move to next part of the string if needed to be written out on seperate line
      nstart += n;
    ...
    }while(nstart < totallength);  // End of loop over fits lines of current entry
    curhistoryEntry = curhistoryEntry->nextEntry;  // Go to next history entry
  }while(curhistoryEntry != NULL); // End of loop over history entries
  return 1;
}
...
2 = Expected HDU is not there
3 = Expected column is not in table
4 = Table contains 0 rows
5 = read error
...
int readHistoryFITS_psrsalsa(datafile_definition *datafile, int surpressHDUerror, verbose_definition verbose)
{
  ...
  fits_get_num_rows(datafile->fits_fptr, &nrows, &status);
    /*    printf("%ld rows in history table.\n", nrows); */
  ...
  for(rownr = 0; rownr < nrows; rownr++) {
    // Make new history entry, unless first entry is not filled yet.
...
/* Shows the contents of the history table. 
   Returns 1 on success, 0 on error */
int readHistoryFITS(datafile_definition *datafile, verbose_definition verbose)
{
  ...
  if(fits_get_hdu_num(datafile->fits_fptr, &hdunum)) { // Always returns a non-zero number?
    //    fflush(stdout);
    //    printerror(verbose.debug, "ERROR readHistoryFITS (%s): Cannot obtain current HDU (set to %d)", datafile->filename, hdunum);
    //    return 0;
  }
  ...
  if(ret == 0 && ret2 == 0)
    return 1;
  /* It is ok when one HDU couldn't be found */
  if(ret == 0 && ret2 == 2)
    return 1;
  if(ret == 2 && ret2 == 0)
    return 1;
  ...
  /* It is ok when one HDU doesn't have the required columns, if the other one worked */
  if(ret == 3 && ret2 == 0)
    return 1;
  return 0;
}
