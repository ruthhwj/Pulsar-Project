...
//START REGION RELEASE
  ...
  nohead = 0;
  noweights = 1;   // By default, don't read in weights
  showfootnotes = 1;
  footnote_length = 0;
  footnote_length2 = 0;
  footnote_search = 0;
  ...
  if(argc < 2) {
	...
	printf("bw            Bandwidth\n");
//START REGION DEVELOP
	printf("cableswap     Cables swapped during obs.\n");
	printf("cableswapcor  Cable swap corrected.\n");
//START REGION RELEASE
  ...
  /* Do an initial check if header parameter is recognized. Can avoid frustration when many files are queried */
  if(c_index != 0) {
    pickWordFromString(argv[c_index], 1, &nrwords, 0, ' ', application.verbose_state);
    //  printf("XXXX: string = '%s' with %d words\n", argv[c_index], nrwords);
    for(i = 0; i < nrwords; i++) {
...
//START REGION DEVELOP
      }else if(strcasecmp(cmd, "cableswap") == 0) {
      }else if(strcasecmp(cmd, "cableswapcor") == 0) {
//START REGION RELEASE
      }else {
  ...
  while((filename_ptr = getNextFilenameFromList(&application, argv, application.verbose_state)) != NULL) {
    ...
    //    if(preprocessApplication(&application, &datain[i]) == 0) 
    //      return 0;

    /* Read header */
    verbose_definition verbose2;
    copyVerboseState(application.verbose_state, &verbose2);
    if(c_index == 0)   // If not querying a specific parameter, dump all parameters
      verbose2.verbose = 1;
  ...
  if(c_index == 0) {  // If we didn't ask for a specific parameter, we're done.
...
//START REGION DEVELOP
	...
	printf(" cableswp_cor");
//START REGION RELEASE
      }else if(strcasecmp(cmd, "hist") == 0) { /* Already taken care of */
      }else if(strcasecmp(cmd, "weights") == 0) { /* Already taken care of */
      }else if(strcasecmp(cmd, "freqlist") == 0) { /* Already taken care of */
      }else if(strcasecmp(cmd, "observatory") == 0) {
	printf(" observatory");
  ...
  */
  ...
  for(index = 0; index < numberInApplicationFilenameList(&application, argv, application.verbose_state); index++) {
	...
	if(strcasecmp(cmd, "p0") == 0 || strcasecmp(cmd, "period") == 0) {
	  ...
	  if(period > 0) {
	    //	    printf(" %7.4lf", get_period(datain[index], 0, application.verbose_state));
	  ...
	  //	  printf(" %9.6lf", get_tsamp(datain[index], 0, application.verbose_state));
...
//START REGION DEVELOP
	  ...
	  printf(" %8d", datain[index].cableSwap);
	}else if(strcasecmp(cmd, "cableswapcor") == 0) {
	  printf(" %12d", datain[index].cableSwapcor);
//START REGION RELEASE
	}else if(strcasecmp(cmd, "hist") == 0) {
	}else if(strcasecmp(cmd, "weights") == 0) {
	}else if(strcasecmp(cmd, "freqlist") == 0) {
